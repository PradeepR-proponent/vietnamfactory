<template>
       <section class="buy-banner">
        <div class="container">
            <div class="row">
                <div class="col-lg-6">
                    <div class="heading-wraper">
                        <h1 class="main-heading">FIND THE NEXT SUPPLIER/BUYER</h1>
                    </div>
                </div>
                <div class="col-lg-6">
                    <button type="submit" class="btn-primary">Get Started Now</button>
                </div>
            </div>
        </div>
    </section>
     <footer class="global-space">
        <div class="container">
            <div class="row">
                <div class="col-lg-4">
                    <div class="row">
                        <div class="col-md-12">
                            <span class="heading">NAVIGATION</span>
                        </div>
                        <div class="col-md-6">
                            <ul class="list">
                                <li class="item"><a href="" class="link">About Us</a></li>
                                <li class="item"><a href="" class="link">Your Feedback</a></li>
                                <li class="item"><a href="" class="link">Consultant Services</a></li>
                                <li class="item"><a href="" class="link">Our Security</a></li>                                
                            </ul>
                        </div>
                        <div class="col-md-6">
                            <ul class="list">
                                <li class="item"><a href="" class="link">Pricing</a></li>
                                <li class="item"><a href="" class="link">Privacy Policy</a></li>
                                <li class="item"><a href="" class="link">SGS/TUV Support</a></li>
                                <li class="item"><a href="" class="link">Contact Us</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="row">
                        <div class="col-md-12">
                            <span class="heading">CATEGORIES</span>
                        </div>
                        <div class="col-md-5">
                            <ul class="list">
                                <li class="item"><a href="" class="link">Medical Supplies</a></li>
                                <li class="item"><a href="" class="link">Food & Agriculture</a></li>
                                <li class="item"><a href="" class="link">Industrial Products</a></li>
                            </ul>
                        </div>
                        <div class="col-md-7">
                            <ul class="list">
                                <li class="item"><a href="" class="link">Fashion & Accessories</a></li>
                                <li class="item"><a href="" class="link">Cosmetic & Personal Care</a></li>
                                <li class="item"><a href="" class="link">Furniture & Home Appliances</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="row">
                        <div class="col-md-12">
                        </div>
                        <div class="col-md-6">
                            <span class="heading">CONTACT US</span>
                            <ul class="list">
                                <li class="item"><a href="" class="link">VIETNAM OFFICE</a></li>
                                <li class="item"><a href="" class="link">JAPAN OFFICE</a></li>
                                <li class="item"><a href="" class="link">GET IN TOUCH!</a></li>
                            </ul>
                        </div>
                        <div class="col-md-6">
                            <span class="heading">FOLLOW US</span>
                            <ul class="list d-flex justify-content-between flex-wrap">
                                <li class="social-link"><a href=""><i class="fa-brands fa-whatsapp"></i></a></li>
                                <li class="social-link"><a href=""><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li class="social-link"><a href=""><i class="fa-brands fa-instagram"></i></a></li>
                                <li class="social-link"><a href=""><i class="fa-brands fa-twitter"></i></a></li>                               
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <div class="footer-bottom">
                <div class="row">
                    <div class="col-lg-6">
                        <img src="assets/img/logos/footer-logo.png" alt="" class="img-fluid footer-logo">
                    </div>
                    <div class="col-lg-6">
                        <span class="copyright-text">© 2023 Vietnam Factory.</span>
                    </div>
                </div>
            </div>
        </div>
    
        <div class="go-to-top">
            <div class="whatsapp">
               <a href=""><img src="assets/img/icons/whatsapp.png" alt="" class="img-fluid"></a>
            </div>  
            <a href="#top">
                <div class="sticky-wraper">
                    <i class="fa-solid fa-up-long"></i>
                </div>
            </a>
            <span class="text">TOP</span>
        </div>

        <div class="pop-cta">
            <button type="button" class="btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Get Best Price</button>
        </div>

    </footer>
</template>

